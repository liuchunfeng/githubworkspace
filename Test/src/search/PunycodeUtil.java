...
public class PunycodeUtil {
	  ...
	  private static final String input_string_null =
	   ...
	   * chstr is the chinese str to be translated can not be null
	   * */
	  public static String chinese2punycode(String chstr) throws PunyException {
	    ...
	    Matcher matcher = CN_MATCH_PATTERN.matcher(chstr);
		 if(!matcher.matches()) return chstr;
	  ...
	  /**
	   * change punycode string to chinese string
	   * punystr is the punycode str to be translated can not be null
	   * */

	  public static String punycode2chinese(String punystr) throws PunyException {
		  Matcher matcher = PUNYCODE_MATCH_PATTERN.matcher(punystr);
		  if(!matcher.matches()) return punystr;
	  ...
	  /** this method transfer unicode(U+****U+****...) into characters
	  ...
	  /**check weather the string is a domain name string
	   * input  inputString
	   * return true false
	   * the input string cant be null
	   * */
	  ...
	  /*** Main_encode function s_input begins with "u+"
	   * Modified by LiuYu 2004-11-19
	   * if fail to transfer , throw PunyExceptions
	   * */
	  private static String punycode_encode(String s_input) throws PunyException {
	    ...
	    /**
	         e_out = puny_prefix + new String(output, 0, output_length);
	         return e_out;
	     */
	   ...
	   *cdns is the string to be translated , can not be null
	   * Modified by LiuYu 2004-11-19
	   * if fail to transfer , throw PunyExceptions
	   * */
	  ...
	  /***
	   * this method transfer punycode(s) to original form
	   * -----characters
	   * if fail to transfer  ,return ""
	  ...
	  private static String punys_to_ocdn(String punys) throws PunyException {
	    ...
	    while (rds.indexOf(dndot) != -1) {
	      ...
	      if (! (tds.equalsIgnoreCase(""))) {
	        /**update for xn--��Сд�޹�*/
	        if (isdomain(tds) && ((tds.length()< puny_prefix.length()) || (!tds.substring(0,puny_prefix.length()).equalsIgnoreCase(puny_prefix)))) {
	          rtds_u = tds;
	        }
	        else {
	          try {
	...
	/**
	      if ((rds.indexOf(puny_prefix) == -1)&&isdomain(rds)) {
	        rtds_u = rds;
	      }
	*/
	      else {
	  ...
	  /**Important Function
	   * s_input is a punycode string can not be null
	   * s_input will not contain a "."
	   * */
...
//		      throw new PunyException(punystr_bad_input);;
	  ...
	  private static final String print_ascii =
	      "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_` "
	      + "abcdefghijklmnopqrstuvwxyz{|}~";
	  private static final String espec_str =
